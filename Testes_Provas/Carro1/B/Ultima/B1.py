...
def Calc_direction(error):
	if(error-OFFSET>0):
		return ((error-OFFSET)*((error-OFFSET)/3))*Kp
	else:
		return (-((error-OFFSET)*((error-OFFSET)/3))*Kp)
	#return (error-640)*Kp
	...
	############### NORMAL ###########
		...
		if(n or n1):
			...
			if(n1>4):
				...
				tempo_passadeira = datetime.datetime.now()+ datetime.timedelta(seconds = 2)
				#First_time = False
			##### SE SAIR PASSADEIRA #####
				...
				if(datetime.datetime.now()>tempo_passadeira):
					Passadeira_Encontrada=False
					Parque = False
					OFFSET=540 # 700 esquerda 540 direita
					t0 = time.time()
					Tempo = True
