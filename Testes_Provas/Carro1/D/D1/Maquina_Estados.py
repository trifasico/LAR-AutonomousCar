...
############### NORMAL ###########
	...
	if(n or n1):
		...
		if(n1>4):
			...
			soma_y/=count
			if(STOP):
				dst_passadeira=math.sqrt((soma_x-640)**2+(soma_y-720)**2)
				if(dst_passadeira<480):
					arduino.write("V.-1".encode())

			Direcao = Calc_direction(soma_x)
			tempo_passadeira = datetime.datetime.now()+ datetime.timedelta(seconds = 3)
		##### SE SAIR PASSADEIRA #####
		elif(Passadeira_Encontrada and (Seta_Direita or Seta_Direita)):
			Set_state(1)
			if(Seta_Direita):
