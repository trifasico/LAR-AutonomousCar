...
void loop() {
  ...
  while(Serial.available()>0){
    ...
    if(Byte!='\r'){
      ...
      char event=0;
      int arg1=0, arg2=0;
      int n = sscanf(Received, "%c.%d.%d", &event, &arg1, &arg2 );
      //Serial.println(event);
      switch(event){
        // === Velocity ===
        case 'V':{
          if(n==2){ // Check received arguments
          
          //range limitation
          if(arg1>100){ arg1=100;}
          else if(arg1<-100){arg1=-100;}
          // motor 
          if(arg1<0){
            
            Velocity = -arg1;
            
            //map(arg1, 0, -100, 0, 255);
        ...
        // === Low Battery ===  (0 - 255) == (0V - 25,5V) === // Tested
        case 'L': {
          if(n==2){  
